...
class BeamSearchDecoder(object):
  ...
  def decode(self):
    ...
    while True:
      ...
      if FLAGS.single_pass:
        decoded = self.write_for_rouge(original_abstract_sents, decoded_words, counter) # write ref summary and decoded summary to file, to eval with pyrouge later
  ...
  def write_for_rouge(self, reference_sents, decoded_words, ex_index):
    ...
    while len(decoded_words) > 0:
      ...
      sent = decoded_words[:fst_period_idx] # sentence up to and including the period
      decoded_words = decoded_words[fst_period_idx+1:] # everything else
      decoded_sents.append(' '.join(sent))
    # pyrouge calls a perl script that puts the data into HTML files.
    # Therefore we need to make our output HTML safe.
    decoded_sents = [make_html_safe(w) for w in decoded_sents]
    decoded_sents = '. '.join(decoded_sents) + '.'
  

    # Write to file
    decoded_file = os.path.join("stories_output/summaries", "%06d_decoded.txt" % ex_index)
